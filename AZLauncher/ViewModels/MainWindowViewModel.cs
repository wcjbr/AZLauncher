...
public sealed partial class MainWindowViewModel : LocalizedViewModelBase
{
    ...
    private readonly VersionSelectPageViewModel versionSelectPage;
    ...
    private bool suppressInstanceSync;
    ...
    public MainWindowViewModel(AppConfigService configService) : base(new LocalizationService(configService))
    {
        ...
        versionSelectPage = new VersionSelectPageViewModel(Localizer, launcherState);
    ...
    public string InstancePickerLabel => IsChinese ? "实例选择" : "Instance";

    public string SidebarActiveSectionLabel => IsChinese ? "当前区域" : "Current section";

    public string SidebarInstanceSummary => IsChinese
        ? $"当前实例：{ActiveProfile}"
        : $"Current instance: {ActiveProfile}";
    ...
    public string VersionSelectNavLabel => IsChinese ? "版本选择" : "Version Select";
    ...
    public bool IsOverviewSelected => CurrentSection == LauncherSection.Overview;

    public bool IsVersionSelectSelected => CurrentSection == LauncherSection.VersionSelect;

    public bool IsDownloadsSelected => CurrentSection == LauncherSection.Downloads;

    public bool IsInstancesSelected => CurrentSection == LauncherSection.Instances;

    public bool IsLibrarySelected => CurrentSection == LauncherSection.Library;

    public bool IsBackupsSelected => CurrentSection == LauncherSection.Backups;

    public bool IsSettingsSelected => CurrentSection == LauncherSection.Settings;
    ...
    public string HeaderEyebrow => CurrentSection switch
    {
        ...
        LauncherSection.VersionSelect => IsChinese ? "版本选择" : "Version selection",
    ...
    public string HeaderTitle => CurrentSection switch
    {
        ...
        LauncherSection.VersionSelect => IsChinese ? "单独切换实例与游戏版本" : "Switch instances and game versions in one place",
    ...
    public string HeaderSummary => CurrentSection switch
    {
        ...
        LauncherSection.VersionSelect => IsChinese
            ? "这里单独展示所有可切换版本，包括从多个游戏目录探测到的实例。"
            : "This page lists every switchable version, including instances detected from multiple game folders.",
    ...
    {
        if (suppressInstanceSync || value is null)
        {
            return;
        }
    ...
    partial void OnCurrentSectionChanged(LauncherSection value)
    {
        RaiseProperties(
            nameof(IsOverviewSelected),
            nameof(IsVersionSelectSelected),
            nameof(IsDownloadsSelected),
            nameof(IsInstancesSelected),
            nameof(IsLibrarySelected),
            nameof(IsBackupsSelected),
    ...
    protected override void OnLanguageChanged()
    {
        ...
        RaiseProperties(
            ...
            nameof(PlayerLabel),
            nameof(InstancePickerLabel),
            nameof(SidebarActiveSectionLabel),
            nameof(SidebarInstanceSummary),
            nameof(LaunchState),
            nameof(OverviewNavLabel),
            nameof(VersionSelectNavLabel),
    ...
    [RelayCommand]
    private void NavigateVersionSelect()
    {
        CurrentSection = LauncherSection.VersionSelect;
        CurrentPage = versionSelectPage;
        RaiseHeaderProperties();
    }
    ...
    private void RefreshInstanceSelections()
    {
        AvailableInstances = launcherState.GetLocalizedInstanceSelections(IsChinese);
    }

    private void SyncSelectedInstance()
    {
        suppressInstanceSync = true;
    ...
    private void HandleLauncherStateChanged(object? sender, PropertyChangedEventArgs e)
    {
            ...
            or nameof(LauncherStateService.Instances))
        {
            RefreshInstanceSelections();
            SyncSelectedInstance();
            RaiseProperties(nameof(ActiveProfile), nameof(StorageValue), nameof(LaunchState), nameof(SidebarInstanceSummary));
        }
    }
}
